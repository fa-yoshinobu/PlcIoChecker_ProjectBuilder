...
- App: Excel 貼り付けが 1 行のエラーで全体失敗しないようにしました。エラー行とスキップ行はプレビューで除外され、OK の行だけを取り込めます。
- App: 全体の配色コントラストを改善しました。補助テキスト・枠線・交互行・選択行を濃くし、これまでほぼ見えなかったセル選択のハイライトを表示するようにしました。
- App: 貼り付けプレビューの状態列をバッジ表示にし、エラー行とスキップ行へ背景色を付けて見分けやすくしました。

### Added

- App: List / Time Chart / Trap / Comment への Excel 貼り付け時に、行ごとの検証結果(OK / エラー理由 / スキップ)を確認してから取り込める貼り付けプレビューダイアログを追加しました。
- App: 貼り付けを Ctrl+Z で取り消せるようにしました。上書きされた行と末尾に追加された行を貼り付け前の状態へ戻します。
- App: タブ内のコンテンツが UI Automation(スクリーンリーダー等)から見えなかった問題を修正しました。
- App: 各タブに「Excel ▾」メニューボタンを追加しました。ショートカットを知らなくても貼り付け / 選択行コピー / テンプレート(ヘッダー行)コピーを実行できます。
